...
#
# 支持架构：aarch64 (arm64-v8a)、armv7a(armeabi-v7a/arm32) 和 x86_64
# 输出目录：/output
#
# 特性：
# - 增量编译（检测源码变化）
...
case "${TARGET_ARCH}" in
    ...
    *)
        ...
        echo "Supported: aarch64, armv7a(arm32), x86_64"
        exit 1
        ;;
esac
echo ""
...
write_talloc_cross_answers() {
    ...
    case "${TARGET_ARCH}" in
        ...
        armv7a|arm32)  uname_machine="arm" ;;
...
if [ -f "${LOADER_OUTPUT}" ]; then
    ...
    case "${TARGET_ARCH}" in
        ...
        armv7a|arm32)  expected_loader_desc="ARM" ;;
...
# ============ 编译 proot-loader32（**仅 aarch64**，arm32/x86_64 不编译） ============
