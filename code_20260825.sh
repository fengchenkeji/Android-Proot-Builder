# 1. 删除错误旧镜像
docker rmi -f proot-builder:armeabi-v7a

# 2. 强制覆盖写入修复后的 build-proot.ps1
cat > build-proot.ps1 <<'PS1_EOF'
<#
.SYNOPSIS
...
.DESCRIPTION
    ...
    - 包名无关：不做包名替换
.PARAMETER Arch
    Target architecture: arm64, arm32, x86_64, or all (default: all).
.PARAMETER Mode
    Build mode:
...
.EXAMPLE
    ...
    .\build‑proot.ps1 ‑Arch arm64,arm32         # build arm64 + arm32
...
if ($allSuccess ‑and $CopyToJniLibs) {
    ...
    foreach ($arch in $archList) {
        ...
        $copyFiles = @('libproot.so', 'libproot‑loader.so')
        if ($arch ‑eq 'arm64') {
            $copyFiles += 'libproot‑loader32.so'
        }
        foreach($fn in $copyFiles){
            $sourceFile = Join‑Path $sourceDir $fn
            if (Test‑Path $sourceFile) {
                Copy‑Item ‑Force $sourceFile $targetDir
                Write‑Success "Copied: $targetDir\$fn"
            }
        }
    }
}
...
if ($allSuccess ‑and $CopyToAssets) {
    ...
    foreach ($arch in $archList) {
        ...
        $targetDir = Join‑Path (Resolve‑Path $AndroidProjectRoot).Path "app\src\$flavorDir\assets\proot\$abiName"
        if (Test‑Path $sourceFile) {
            New‑Item ‑ItemType Directory ‑Force ‑Path $targetDir | Out‑Null
            Copy‑Item ‑Force $sourceFile $targetDir
...
# 3. 校验文件参数列表是否正确
grep ValidateSet build-proot.ps1
